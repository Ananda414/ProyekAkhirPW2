...
@section('title', 'Halaman Simplisa')
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                @endif
                <h4 class="card-title">Daftar Simplisa</h4>

                <!-- Tambahkan input filter -->
                <input type="text" id="filterInput" class="form-control mb-3" placeholder="Filter nama simplisa...">
                ...
                <div class="table-responsive">
                    <table class="table table-striped" id="simplisaTable">
                        <thead>
                            <tr>
                                <th>No.</th>
                                <th>Nama Barang/Simplis</th>
                                <th>Details</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($simplisas as $item)
                                <tr class="main-row">
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                          <a href="{{ route('simplisa.edit', $item->id)}}">
                                            @can('update', $item)
                ...
                @if ($simplisas instanceof \Illuminate\Pagination\AbstractPaginator)
                    <div class="pagination-wrapper">
                        {{ $simplisas->appends(['per_page' => request('per_page')])->links('pagination::bootstrap-4') }}
                    </div>
                @endif

                <a href="{{ route('simplisa.downloadPDF') }}" class="btn btn-info">Download PDF</a>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#filterInput').on('keyup', function() {
            ...
            $('#simplisaTable tbody tr.main-row').each(function() {
